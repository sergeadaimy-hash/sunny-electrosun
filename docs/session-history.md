...
**Same-day follow-up (still 2026-08-18): setup executed + Calls tab shipped.** Serge approved and everything through "ready to test" was done in-session: Deepgram + Cartesia keys received (stored in `~/Desktop/ElectroSun-Voice-Credentials.md`, both pasted in chat so rotate after the soak), `VOICE_SERVICE_SECRET` generated and set on both services, the `sunny-voice` Railway service created VIA CLI (`railway add -s sunny-voice -v ...`, then `railway up voice --path-as-root --service sunny-voice`; the FIRST deploy accidentally built the repo root = a rogue Node copy, caught via the /health shape and fixed by `--path-as-root`), domain `https://sunny-voice-production.up.railway.app`, `calls` webhook field subscribed programmatically (POST `/{app-id}/subscriptions` with app token, fields `messages,calls`, app id `2440193806402796`), Serge flipped "Allow voice calls" ON in WhatsApp Manager, and `VOICE_SERVICE_URL` set on the Node service. Outbound calling (Sunny calls Patrick) was investigated and DECLINED for now: Pipecat's WhatsApp transport is inbound-only, and Meta requires a user-approved call-permission flow + per-minute billing for business-initiated calls; testing is done by calling IN.

Then Serge asked for a **Calls tab in the admin with transcripts** (tests 344/344): new `voice_calls` table (migration in `db/init.js`, unique on `wa_call_id`); `src/voice_calls.js` store (`recordVoiceCall` upsert + contact link + one outbound `voice_call` thread marker safe from the orphan sweep, `listVoiceCalls`, `getVoiceCallById`, `sanitizeTranscript`); `POST /voice-transcript` on the main app (X-Voice-Secret auth, constant-time); `GET /api/calls` + `/api/calls/:id` (admin role only); admin "Calls" tab (list + click-to-expand bubble transcript); `voice/bot.py` collects the LLM context at call end and posts it best-effort (`SUNNY_BASE_URL`). Tests: `test/voice_calls.test.js`.
...
What shipped (suite 251/251, 15 new tests in `test/vault.test.js`):
- **`vault/` at repo root**: `products/` (inverters, batteries, panels, accessories), `policies/` (warranty, delivery, payment-plans, pricing POLICY only), `playbook/` (objections, escalation-rules, language-notes), all seeded as plain-text templates with `[TODO: ...]` fill-in lines and `%%...%%` editing-instruction blocks. `vault/tag-map.json` maps each topic tag to its file, a description (fed to the classifier), and fallback keywords. Adding a topic = add a file + a map entry, no code change.
- **`src/vault.js`**: fail-open loader. `buildKnowledgeBlock(tags, messageText)` reads only the mapped files for the sanitized tags (keyword fallback on the message when the classifier gave none), strips `%%` blocks and `[TODO` lines (an unfilled template injects NOTHING), enforces a hard budget (`VAULT_PROMPT_BUDGET_TOKENS`, default 1000 tokens, truncation at line boundaries), refuses path traversal, and wraps the result in `<business_knowledge>` with an explicit "prices and stock still come ONLY from Warehouse Stock" preamble. `buildClassifierTagBlock()` renders the tag list for the classifier from the same map.
- **Classifier extension** (`src/claude.js > classify`): a third cached system block (after classifier.md, before warehouse) instructs the model to add `topic_tags` (0 to 3, fixed list) to its JSON. Built in code from tag-map.json, NOT edited into classifier.md, so an owner prompt rewrite cannot drop the field. `FALLBACK_CLASSIFICATION` and the greeting fast-path carry `topic_tags: []`; parsed tags are sanitized against the map (unknown tags dropped).
- **Reply injection** (`src/claude.js > generateReply`): the `<business_knowledge>` block is pushed AFTER the 4 cached blocks (system/playbook/facts/warehouse, which already use all 4 cache breakpoints, so the vault block is deliberately uncached), skipped on the greeting fast-path. All three `generateReply` call sites in `processCustomerBatch` pass `topicTags: classification.topic_tags`. Every injection logs `vault.injected` with tags + estimated tokens so savings are measurable.
- **`VAULT.md`** at repo root documents the layout, the tag map, the editing rules for the brother (plain text, no prices, no account numbers, no double dashes), and the env vars (`VAULT_DIR`, `VAULT_PROMPT_BUDGET_TOKENS`, `VAULT_MAX_TOPIC_FILES`).
...
**Root cause of the silence (confirmed in Railway logs):** the warranty question opened pending query 1059; `handler.followup_reply_suppressed_silence_cooldown` then swallowed both follow-ups (12:16:13, 12:16:59) and both orphan-sweep retries (12:20, 12:25, after which `ORPHAN_RECOVERY_MAX_ATTEMPTS=2` capped out). The `topicShiftAutoResolve` guard that exists precisely to prevent this matched NOTHING: "640w" is watts (the pattern only knew kw/kva/kwh), "how much" and "N pieces" were not topic-shift patterns at all. A 10-piece buying signal was silenced by an unrelated open warranty query.

**Fix:** four patterns added to `TOPIC_SHIFT_PATTERNS` (price asks how much/price/cost/quote, wattage `\d+w`, quantities pieces/pcs/units/sets/nos, "i need/want/will take N"); `isLikelyTopicShift` exported; `test/topic_shift.test.js` (7 tests incl. Frank's exact messages as regression; nags like "any update?" still stay on the follow-up path). After deploy the orphan sweep re-queues Frank's unanswered inbound automatically (the in-memory attempt cap resets on restart) and the topic shift now resolves the stale query so he gets a real reply.

**Vault pre-fill:** all files except warranty populated from EXISTING agent knowledge only (nothing invented): product families + engineering rules (parallel same-size max 10, LV/HV pairing, BOS-A 7.68kWh / BOS-B Pro 16kWh, HV cluster floors 5/7/7, LV ceilings 32 packs / 10 inverters), delivery/pickup/export doctrine, account-details-only-via-Sales-Manager, the full discount/bulk policy, objection scripts, escalation judgement calls, language/tone rules. Unknown facts (warranty terms, payment plans, pickup hours, price-match line) stay as `[TODO` lines that inject nothing, so warranty questions still correctly escalate. Injection sizes: 185-466 tokens per topic, worst-case 3-topic block 962 (budget 1000). Repo-vault test now asserts filled content + no TODO/scaffolding leaks + budget. Suite 258/258.
...
**1. Image-reading fix (`aa11df2`).** Root cause traced live in production logs (contact 263, 08:47 Jul 11): customer sent a product photo captioned "I mean this one"; the image downloaded fine, but the text-only classifier saw just `[Customer sent an image with caption]`, classified `silent_query`, alerted the Abuja desk, and the pending-query silence cooldown then suppressed the customer reply COMPLETELY, so no model ever looked at the image. Three-part fix: (1) `describeInboundImage` in `src/claude.js` runs one strictly-descriptive vision call per downloaded image (MODEL_CLASSIFIER, max 200 tokens, quote legible text verbatim, never guess, budget-guarded, null -> legacy marker fallback); the description travels in the persisted body and classifier input (`buildImagePersistedBody` / `buildImageCombinedPart`), so classification, alerts, pending queries, and future history all see the image content; (2) image-bearing turns bypass the silence cooldown (`shouldSuppressFollowupReply`, extracted pure + tested); (3) the awaiting-expert context gains a `hasImage` instruction (answer what the image shows, match legible models against Warehouse Stock, never guess specs from an unclear photo), and `classifier.md` states an attached image is never by itself an escalation. LIVE VERIFICATION PENDING: send a photo to the production number and check the reply references the image content plus `handler.image.described` in logs.

**2. Dependencies (`9ab1bdd`).** `npm audit` was 7 vulns (4 high): axios, form-data, qs, tmp fixed in-range; nodemailer 8.0.11 -> 9.0.3 (only createTransport/sendMail used); uuid forced to ^11.1.1 under exceljs via npm `overrides` (instead of npm's suggested exceljs DOWNGRADE). exceljs xlsx generation and nodemailer transport smoke-tested. Audit now clean.

**3. API auth hardening (`028de76`).** `/api/*` had unlimited key guessing and plain `===` compares. Now `security.safeKeyCompare` (sha256 + timingSafeEqual) for master + inbox keys, and a per-IP FAILED-attempt throttle (20/10min, `API_AUTH_MAX_FAILURES`/`API_AUTH_WINDOW_MS`); a valid key always passes so a shared proxy IP cannot be used to lock out the admin; blocked guesses get 429 + `security.api_auth_throttled`.

**4. Nightly DB backup (`bba023d`).** Production had ZERO backups (snapshotDb() lives on the report cron, silenced by `DISABLE_NOTIFICATIONS=true`; `/data/sunny.db` was a single copy). New `src/db_backup.js`: better-sqlite3 online snapshot -> gzip -> `/data/backups`, keep newest 14 (`DB_BACKUP_DIR`/`DB_BACKUP_KEEP`), cron `10 2 * * *` Africa/Lagos, ALWAYS registered. Volume-loss protection still needs Railway volume backups or an off-platform copy (follow-up option).

**5. Admin XSS sweep (`f63c060`).** Every HTML-building site in `public/admin.html` audited: customer-controlled content already escaped at assignment, no inline handlers, URLs built from ids + encodeURIComponent. One gap fixed: `lead_temperature`/`category`/`client_type` pills rendered raw in three builders; these come from parsed LLM classifier JSON (second-order injection channel), now escaped.

Also noted for follow-up (from the same review, NOT implemented): classifier -> Haiku 4.5 trial, classifier history trim, system.md diet (owner approval needed), OpenAI key rotation (Serge's dashboard), `?key=` query param replacement with signed URLs, `DAILY_LLM_BUDGET_USD` 50 -> 30 decision.
...
**1. Solar Analyst (2349159721115) incident.** Two defects: (a) "I wanted to pay for the DEYE 5KWH" did NOT force-promote to HOT. `HOT_TRIGGER_RE` knew "want to pay"/"wants to pay" but not the past tense; the turn rode the generic follow-up path (contact stayed COLD, the Lagos desk got FOLLOW-UP NEEDED instead of HOT LEAD). Fixed: regex now covers `want(s|ed) to pay`, `would like to pay`, `i wan pay` (Pidgin), `how/where do|can i|we pay`, `make (the) payment`, `account to pay (into)`, `pay into`. `hasHotTrigger` exported + new `test/hot_trigger_payment.test.js`. (b) The Jul 4 turn "Location" + "Account to pay into" was never answered at all: a redeploy wiped the in-memory debounce queue and boot-time `recoverOrphanedInbound(10)` only looks back 10 minutes, so the turn was lost until the next inbound a day later. Fixed with a **periodic orphan sweep** on the always-on `*/5` cron: `recoverOrphanedInbound(360, { minAgeMinutes: 3 })` re-queues any inbound from the last 6h with no outbound after it, at least 3 min old (never races the debounce window or an in-flight LLM call), excluding reactions (`intent='reaction'` / `[reacted:` bodies, which legitimately get no reply), human_handled conversations, and team numbers, capped at 30/run. A lost turn now self-heals within 5 minutes.

**2. Charbel (an owner) was tagged HOT/SERIOUS/RESIDENTIAL in admin.** Root cause: the full-owner branch in the dispatch loop only handled `msg.kind === 'text'`; an owner VOICE NOTE or image fell through into the customer pipeline and got classified as a lead. Fixed: the owner branch now ends in an unconditional `continue`. Owner voice notes are downloaded + Whisper-transcribed and answered via Owner Q&A (owners can now ask by voice); other owner media is persisted (`intent='owner_media'`) with no reply and no classification. Plus `scrubTeamContactLeadTags()` at every boot: nulls category/lead_temperature/client_type on any contact whose digits match `teamPhoneDigits()`, so the stale chips disappear on next deploy and can never come back.

**3. Owner Q&A upgrade (owner directive: answer ANYTHING the two owners ask, in detail).** `handleOwnerNonQueryMessage` now has a capability layer: (a) **datasheet fast-path**: an owner asking for a datasheet/brochure/spec gets the actual file via the same warehouse matcher + Meta upload used for customers (`intent='owner_datasheet_sent'`); (b) **transcript fast-path**: "send his chat" / "show me the conversation" sends the customer's transcript verbatim (`formatConversationBriefForOwner`, 40 turns, chunked at 3500 chars, `intent='owner_transcript'`). The target customer resolves from an explicit phone number in the owner's message, else from the last customer number mentioned in the owner's own thread (so "send his chat" right after a hot-lead alert just works); (c) **customer-in-focus context**: any other question naming a customer gets that contact's details + 30-turn transcript injected into the Owner Q&A call (`answerOwnerQuestion(ownerContactId, question, { extraContext })`), `max_tokens` 600 -> 1500, replies chunked. `owner_qa.md` rewritten accordingly: transcripts are now IN capability (the old prompt explicitly said "I can't send chat transcripts from here"), detail-length answers allowed when the owner asks for data, admin dashboard only for genuinely absent things.

New helpers exported from handler.js: `scrubTeamContactLeadTags`, `extractCustomerPhoneDigits` (skips team numbers), `chunkText`.
...
**Delivery blocker found on first live test (2026-06-24):** template sends are accepted by the API (`whatsapp.template.ok`) but FAIL at delivery with Meta `error_code 131042 "Business eligibility payment issue"` (`whatsapp.delivery.failed` on Railway), to BOTH Patrick and the developer line. Root cause: the WABA (`986225450549617`) has no valid payment method attached, so WhatsApp rejects every BILLABLE message = all template sends. Free-form service replies (free, in-window) keep delivering, which is why the customer pipeline always worked and this only surfaced when templates were first used. NOT a MARKETING-category or Message-Requests issue. Fix is a business/account action only Serge can do: WhatsApp Manager / Meta Business Settings -> WhatsApp Accounts -> ELECTROSUN WABA -> Billing/Payment settings -> add a valid payment method, then re-test `node scripts/send_test_escalation.js`. No code/template change needed. Until billing is fixed, `sendOwnerAlert` falls back to free-form (delivers only inside the 24h window, i.e. the original problem persists).

Safe to deploy before approval: if the template is missing / PENDING / rejected, `sendOwnerAlert` falls back to the old free-form send (window-bound). Now that it is approved, owner/sales-desk alerts are window-independent automatically ONCE BILLING IS FIXED. Committed + pushed to main (`3244f6a`); the test script + ElectroLeads handoff doc in `68c8b00`.
...
### Screenshot fixes: owner-routing awareness, live-agent escalation, no customer names (commit `f1ab454`, pushed)

Two screenshots (customer conv 2633 "Ajay"; owner "Electro1" chat) surfaced three more bugs, plus an owner directive on naming.

- **O1 (HIGH, owner misinformation):** the owner asked "have you sent them to the abuja sales contact?" and Owner Q&A replied that forwarding "would need to be set up", when the Abuja desk IS configured and was receiving routed alerts that day. Root: the owner_qa snapshot had no routing facts, so the model guessed. Fix: `buildRoutingSummary(recipients)` (exported from `owner_qa.js`, unit-tested) builds a factual block (routing is active, how leads forward by city, which desks are set, any unset desk flagged) and is injected as `snapshot.lead_routing`; `owner_qa.md` tells the model to answer routing questions only from it and never to say routing needs setting up unless a desk is explicitly unset.
- **C1b (MEDIUM-HIGH, missed handoff):** "Connect me with a live agent" then "Yes" produced no escalation; the customer waited 10 minutes for a manual takeover. New `isLiveAgentRequest(text)` (exported, unit-tested) forces a routed `live_agent` escalation (added to `ESCALATION_HEADERS` + `ROUTING_WORTHY_ESCALATIONS`); region-unknown asks "Abuja or Lagos?" via gather-first, region-known alerts the regional desk and appends the routed Sales Manager direct line.
- **C1a (MEDIUM, vague reply to a clear product ask):** "I need Deye 5.3kwh" (a stocked product) got "Could you share more about your project" because Sunny volunteered a price that the strip guard nuked into the generic fallback. Strengthened `system.md`: a product-interest message with no price word must contain NO price and instead name the product + one qualifier (template + the 5.3kWh example added).
- **Naming (owner directive, "stop calling people by their WhatsApp names, use Sir"):** removed the customer name from the reply context entirely (extracted `buildKnownCustomerContext(contact, isCasualGreeting)` in `src/claude.js`, deleted the two `Customer name:` lines, unit-tested that the name never appears), and rewrote the addressing rule in `system.md` to always use "Sir", never the name, never the WhatsApp profile name, even if the customer states their name. The name is still captured in the DB/admin, just not handed to the reply model.

`test/reply_guards.test.js` now 31 cases (routing summary, live-agent, name-free context added). Suite 80/80. Verified live earlier in the session: R2 silent expiry cleared 244 stale queries (253 -> 10 pending) and the batch instrumentation is logging.

### City-unknown default to Abuja + Sales Manager link regression (commit `ddd817f`, pushed)

Owner re-audit request (worried leads were not reaching the Abuja/Lagos desks). Full cross-check of all 50 of today's conversations against the four desk threads in admin confirmed: desks ARE configured and reachable (Abuja got 2 lead alerts + sent acks, Lagos got 1 + ack, zero delivery failures), and the post-fix proof case is Wizzypro (conv 2629, 15:33, after the R1 deploy): gather-first asked "Abuja or Lagos?", customer said Lagos, alert routed to the Lagos desk. The morning Patrick-heavy alerts all predated the R1 fix (~15:28 deploy). Two follow-up directives from the owner:

1. **Default a city-unknown lead to Abuja, do not leave it waiting or send it to Patrick.** `decideRecipient` gained an `abujaConfigured` input: region-unknown (non-big) now returns the Abuja desk (`reason: region_unknown_default_abuja`) when the Abuja number is set, else still the owner. `resolveRecipient` passes `abujaConfigured = !!numberForLabel('abuja')`. Gather-first now asks the city ONCE: if a `deferred_handoff` is already pending and the customer still gave no city, the escalation routes now (to Abuja) instead of looping on "Abuja or Lagos?"; the flag is cleared on default. Wizzypro-style answered-city leads still route to the named desk via the existing deferred-handoff resume.
2. **Sales Manager wa.me link must appear whenever Sunny refers the customer to the Sales Manager** (it had narrowed to hot_lead/bulk_order/live_agent only). Audit found customer replies mentioning the Sales Manager with NO link (Donwills, Skyline, Delight, Ajay), all region-unknown leads with no resolved recipient. Fix: append the routed desk's direct line whenever the reply matches `/sales manager|specialist/i` AND an escalation resolved a recipient this turn (`isReferralHandoffThisTurn`), generalizing the old type-specific gate. The 142 "with link" the first grep found were owner-alert bodies in Patrick's thread (those carry a follow-up link), not customer replies.

Tests: `test/owner_routing.test.js` +2 (Abuja default with/without config); full suite 82/82.

**Ghost sweep (commit `078d4f0`):** the true-ghost case (asks nothing after the "Abuja or Lagos?" question) is now handled. `routeStaleDeferredHandoffs(thresholdMinutes)` in `src/handler.js` finds contacts whose `deferred_handoff` is older than the threshold (default 5 min, `STALE_HANDOFF_MINUTES`), routes each to the Abuja desk (region-unknown default in `decideRecipient`), alerts the desk, and clears the flag (swept at most once). Routes only, never re-messages the customer. Safety bounds: off when `DISABLE_ESCALATIONS=true`; only sweeps leads from the last 24h (older = dead, ignored, prevents a first-run flood); capped at 30 per run. Wired into the always-on `*/5` cron in `server.js` next to auto-release. Not unit-tested (DB + notify cron plumbing); verified by syntax + suite + the decideRecipient Abuja-default tests, to be confirmed on live logs.
