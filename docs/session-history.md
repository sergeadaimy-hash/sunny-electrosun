...
Tests: added 3 owner_alert cases (message-fallback summary, brief-wins-over-message, long-message truncation) and 3 owner_routing cases (all-four, unset-desk-omitted, dup-deduped). Suite now 50/50 green. All touched modules load; admin.html inline script parses (vm check). No deploy, no commit (per project norm, Serge pushes).
