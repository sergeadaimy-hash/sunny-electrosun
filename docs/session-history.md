...
Tests: `test/owner_routing.test.js` +2 (Abuja default with/without config); full suite 82/82. Still open: a TRUE ghost (sends nothing after the city question, e.g. Donwills) needs a sweep-to-Abuja cron, deferred to its own isolated change because it auto-fires alerts.
