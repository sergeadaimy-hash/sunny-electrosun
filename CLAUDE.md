...
**Source of truth:** https://github.com/sergeadaimy-hash/sunny-electrosun (private). Pushes from Claude's non-interactive shell hang on the credential prompt; Serge pushes manually with `git push` from his Terminal or `! git push` syntax in chat. Latest commit on main as of this snapshot: `81a8b4f` plus an alert-desk-ack commit (see `git log`), pushed + deployed 2026-06-07. Session covered: routed customer link, real 2-phrase owner brief, 4-number Owner Chat tab, price-strip garble fix (`50b7e81`); header badge now "today's HOT leads" not all-time pending (`4218041`); owners/sales excluded from lead stats + Owner Q&A snapshot (`f16bfd6`); bulk-request loop fixed via `bulk_order` escalation = quote unit price + Sales Manager + routed direct line (`81a8b4f`); alert-only sales desks now get a generic throttled ack and their inbound is persisted for admin visibility (still no relay / no Owner Q&A). Alert desks confirmed alert-only + free-form delivery. Use `git log --oneline` for the live tip. See `docs/session-history.md` for what each recent commit / push covers and `docs/audits/2026-05-15-sunny-full-audit.md` for the latest full audit.
