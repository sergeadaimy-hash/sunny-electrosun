...
- **Unproductive conversations are muted (2026-07-11 owner directive).** Non-serviced-script small talk (e.g. Arabic chatter), emoji volleys, and unintelligible dot-transcripts get ONE polite reply per streak, then Sunny goes silent (no classifier, no reply, `silent_skip` marker written). Bare junk links (TikTok/Facebook shares, adult domains) never get a reply at all. Any substantive message (product word, digit, Latin-script text) instantly resets the streak; the 24h conversation rollover restores the allowance. NO closing line is sent when muting. Deterministic, zero-LLM-cost: `src/idle_chatter.js` + `maybeMuteIdleChatter` in `src/handler.js`, tunable via `IDLE_CHATTER_FREE_REPLIES`.
...
| `src/idle_chatter.js` + `src/handler.js > maybeMuteIdleChatter` | Idle-chatter mute (2026-07-11 owner directive: stop replying to unproductive conversations). Pure core `classifyLowValue(text)` tags a message `bare_link` / `emoji_only` / `non_serviced_script` / `unintelligible` or null; `assessIdleChatter` counts the trailing streak of consecutive low-value customer turns and mutes once the polite-reply allowance (`IDLE_CHATTER_FREE_REPLIES`, default 1) is used up. Bare junk links (TikTok/Facebook shares, adult domains, any message that is only a URL) never get a reply at all. The handler wrapper runs in `processCustomerBatch` BEFORE the classifier (muted turn = zero LLM calls), writes the `silent_skip` marker (orphan-sweep safe) + `idle_chatter_muted` event. Never mutes: image-attachment turns, any message with a product token, digit, or majority-Latin text (all five serviced languages AND French are Latin script, so real leads incl. foreign ones always pass). A substantive message resets the streak instantly; 24h conversation rollover gives a fresh allowance. Tests: `test/idle_chatter.test.js`, `test/idle_chatter_wiring.test.js`. |
