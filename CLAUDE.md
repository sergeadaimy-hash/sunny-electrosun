...
- **LLM**: Anthropic Claude API. All call sites (`src/claude.js > classify`, `generateReply`; `src/knowledge.js > extractKnowledge`; `src/owner_qa.js > answerOwnerQuestion`; `src/audit.js`) default to `claude-sonnet-4-6` (2026-07-06 revert from the one-day Sonnet 5 switch) with `thinking: { type: 'disabled' }`. Override via `MODEL_*` env vars. Prompt caching enabled on system blocks via `cache_control: { type: 'ephemeral', ttl: '1h' }` (1-hour TTL since 2026-05-30; see Models, costs, and budget).
