...
- Model assignments (2026-07-12): **classifier runs `claude-haiku-4-5-20251001`** (owner-approved cost trial; ~$1/$5 per MTok vs Sonnet's $3/$15, expected ~$3-5/day saving). Everything else stays `claude-sonnet-4-6`: `MODEL_REPLY`, `MODEL_TEACHER`, `MODEL_OWNER_QA`, `MODEL_AUDIT`, and the new `MODEL_IMAGE_DESCRIBE` (image descriptions feed owner alerts, so they are held on Sonnet independently of the classifier). Safety context for the Haiku classifier: HOT promotion/demotion, greeting/clarification blocks, and the parse fallback are all deterministic code in `src/classifier.js`, and `tryParseJson` strips Haiku's code fences. Rollback: `railway variables --set MODEL_CLASSIFIER=claude-sonnet-4-6`. Watch `claude.classify.parse_fail` rate and escalation quality for 2-3 days. Prior state (2026-07-06 revert): ALL FIVE call sites on `claude-sonnet-4-6` after the one-day Sonnet 5 switch. Reasons for the revert: 2026-07-06 spend hit $32 on FEWER conversations, driven partly by Sonnet 5 classifier JSON truncating at the old `max_tokens: 400` (parse_fail -> paid retry -> fallback), and the owner asked to go back. `MODEL_REPLY`, `MODEL_CLASSIFIER`, `MODEL_TEACHER`, `MODEL_OWNER_QA`, `MODEL_AUDIT` all set to `claude-sonnet-4-6` on Railway (staged with `--skip-deploys`, live on the 2026-07-06 push); code-level fallbacks in `src/claude.js` / `src/knowledge.js` / `src/owner_qa.js` / `src/audit.js` match. Every `messages.create` call keeps `thinking: { type: 'disabled' }` (accepted on Sonnet 4.6, and guards any future model that defaults to adaptive thinking). Classifier `max_tokens` raised 400 -> 1000 (2026-07-06) so the JSON stops truncating. Text extraction stays `content.find(b => b.type === 'text')` everywhere. `src/cost_tracker.js` keeps both `claude-sonnet-4-6` and `claude-sonnet-5` pricing rows ($3/$15 per MTok).
- `DAILY_LLM_BUDGET_USD=20`.
- **Knowledge vault LIVE in code AND pre-filled (2026-07-17).** Files were populated from existing agent knowledge (system.md doctrine, warehouse families, engineering rules) per Serge's directive ("my brother will not fill anything"); per-topic injection runs 185-466 tokens, worst-case 3-topic block 962. Genuinely unknown facts (warranty terms, payment plans, pickup hours) remain `[TODO` lines that inject nothing and still escalate; only the business can supply those. `vault/` at repo root holds plain-markdown business knowledge (products / policies / playbook) mapped by `vault/tag-map.json`. The classifier outputs `topic_tags` (0-3, fixed list, via a code-built cached system block so owner prompt rewrites cannot drop it); `src/vault.js > buildKnowledgeBlock` injects ONLY the matching files into the reply as an uncached `<business_knowledge>` block, hard-capped at `VAULT_PROMPT_BUDGET_TOKENS` (default 1000). Unfilled `[TODO` templates inject NOTHING, so live effect is zero until the brother fills the files. Fail-open everywhere; injections logged as `vault.injected` with token estimates. Docs: `VAULT.md`. Scope decision (Serge, 2026-07-17): topics/knowledge only, NO per-customer vault files, NO idle summarizer. Deferred pending approval: carving system.md content into vault topics.
...
- **Never-silent escalation guarantees (2026-07-05 owner directives; owner runs ads in Cameroon/Niger/everywhere, so foreign region-unknown HOT leads are the norm).** Four hard rules, all in `src/handler.js` + `src/security.js`, tests 157/157: (1) **A HOT commitment never waits for a city.** `gatherFirst` now carries `!isHotEscalation`; a region-unknown HOT lead routes immediately (Abuja desk default) instead of deferring on "Abuja or Lagos?" forever. (2) **Big orders route to OWNERS by BOM total.** `isBigProjectByValue` reads the largest Naira figure in the customer text OR Sunny's own recent BOM (≥ `BIG_PROJECT_NGN_THRESHOLD`, default ₦15M) and sets `routing_category='big_project'` -> Patrick/Charbel round-robin, no region needed. (3) **Never name the Sales Manager/specialist without a link AND a parallel escalation.** A post-reply invariant force-escalates on any "Sales Manager"/"specialist" mention when none fired this turn (`source='sales_manager_referral'`), clears a stale gather-first defer, and ALWAYS appends the direct line (scaled: owner for a big project, Abuja/Lagos desk otherwise). The two handoff backstops lost their `!gatherFirst` guard so a promised handoff always alerts. (4) **Stuck/uncertain replies escalate.** Uncertainty language ("I'm not sure about", "I can't confirm", "I don't have that information") is in `STALL_PATTERNS`, with a negative lookahead so a genuine clarification ("not sure I understand / what you mean / which model") is not flagged; stuck replies run through the stall-guard escalation instead of leaving the customer confused.
- **Unproductive conversations are muted (2026-07-11 owner directive; hardened 2026-07-15).** Non-serviced-script small talk (e.g. Arabic chatter), emoji volleys, and unintelligible dot-transcripts get ONE polite reply per streak, then Sunny goes silent (no classifier, no reply, `silent_skip` marker written). Bare junk links (TikTok/Facebook shares, adult domains) never get a reply at all. A substantive message (product word, digit) instantly resets the streak; the 24h conversation rollover restores the allowance. NO closing line is sent when muting. Deterministic, zero-LLM-cost: `src/idle_chatter.js` + `maybeMuteIdleChatter` in `src/handler.js`, tunable via `IDLE_CHATTER_FREE_REPLIES`. **2026-07-15 hardening (a live spam thread kept getting replies):** (a) link messages are judged on the text AROUND the URLs; a share-card whose surrounding text is itself low-value (Arabic TikTok boilerplate) is a `bare_link` (zero replies), and the script-ratio threshold rose 0.3 -> 0.5; (b) new `courtesy` kind (thanks/greetings/affection with nothing else): it EXTENDS a junk streak instead of resetting it, but is only muted when the streak already contains hard junk, so a real customer's "thank you" after a quote is never touched; (c) transliterated-Arabic chatter ("Ya habibi, kif haalik...", how Whisper-pinned-to-English romanizes Arabic voice notes) is caught by a Latin-token lexicon (2+ strong tokens like habibi/mishtaq/walla, 30% token ratio, digit/product guard first; Nigerian loanwords wallahi/salam alaikum/alhamdulillah verified safe).
...
**Stall-language guard (in `src/handler.js > processCustomerBatch`):** after `generateReply` returns, before sending, `security.detectStallLanguage(reply.text)` checks for first-person stall patterns ("let me check / I'll confirm / will revert / will get back to you / one of our sales engineers will reach out / give me a moment"), third-person handoff phrases ("the team will confirm", "a specialist will reach out", "team member will follow up"), AND (2026-07-05 owner directive: none should go with confusion) **uncertainty / "stuck" language** ("I'm not sure about", "I can't confirm", "I don't have that information", "that's not something I can confirm") so a stuck reply escalates instead of leaving the customer confused. A negative lookahead excludes genuine clarifications ("not sure I understand / what you mean / which model"), which are a legitimate one-question ask, not a stall. The guard only runs when no `expertContext` was already injected (i.e. the LLM was on a normal reply path and stalled anyway). If matched AND `DISABLE_ESCALATIONS=false`, the guard now branches (R1/R3, 2026-06-08):
...
| `MODEL_CLASSIFIER`, `MODEL_REPLY`, `MODEL_TEACHER`, `MODEL_OWNER_QA` | Override the model per call site. Since 2026-07-12: `MODEL_CLASSIFIER=claude-haiku-4-5-20251001` (cost trial), everything else `claude-sonnet-4-6`. Code defaults remain `claude-sonnet-4-6` everywhere, so unsetting a var is always a safe rollback. |
| `MODEL_IMAGE_DESCRIBE` | Model for the per-image vision description (2026-07-12). Defaults to `MODEL_CLASSIFIER` when unset; set to `claude-sonnet-4-6` on Railway so image descriptions (which feed owner alerts and classification) stay on Sonnet while the classifier runs Haiku. |
...
| `BIG_PROJECT_NGN_THRESHOLD` | Naira value at/above which an order is treated as a big project and routed to the OWNERS (Patrick/Charbel round-robin) regardless of region (default `15000000` = ₦15M). `src/handler.js > detectLargeOrderNgn/isBigProjectByValue` reads the largest Naira figure in the customer's text OR Sunny's own recent BOM (last 4 assistant turns; only figures tied to ₦/NGN/naira/"million" count, never SKU/wattage/kWh), and when it clears the threshold sets `classification.routing_category='big_project'` before routing. Shipped 2026-07-05 (Franck/Cameroon ₦41M incident). |
| `META_REGISTRATION_PIN` | Cloud API registration PIN (current: `271828`). Needed if Meta forces re-register of the phone number. |
| `ORPHAN_RECOVERY_MAX_ATTEMPTS` | Max times the orphan sweep re-queues the same inbound message (default `2`). Backstop against intentional-silence turns looping through classify every 5 minutes (the 2026-07-06 cost runaway). |
| `IDLE_CHATTER_FREE_REPLIES` | How many consecutive low-value turns (non-serviced-script chatter, emoji-only, dot transcripts) still get a polite reply before the conversation is muted (default `1`, owner decision 2026-07-11). Bare junk links are muted immediately regardless. Raise to be more forgiving; a large value effectively disables the guard. See `src/idle_chatter.js`. |
| `VAULT_DIR` / `VAULT_PROMPT_BUDGET_TOKENS` / `VAULT_MAX_TOPIC_FILES` | Knowledge vault (2026-07-17). Vault location (default `<repo>/vault`, ships with the code like the prompt files), hard token cap on injected vault content per message (default 1000), max topic files per message (default 3). See `VAULT.md` and `src/vault.js`. |
| `API_AUTH_MAX_FAILURES` / `API_AUTH_WINDOW_MS` | Per-IP brute-force throttle on FAILED `/api/*` key attempts (defaults 20 failures / 600000 ms). Valid keys always pass (no lockout DoS via shared proxy IP); blocked guesses get 429. Key comparison is constant-time (`security.safeKeyCompare`). Shipped 2026-07-12. |
| `DB_BACKUP_DIR` / `DB_BACKUP_KEEP` | Nightly DB backup location and retention (defaults `<DB dir>/backups` = `/data/backups` on Railway, keep 14). Gzipped better-sqlite3 online snapshot at 02:10 Africa/Lagos on an ALWAYS-ON cron (outside the `DISABLE_NOTIFICATIONS` gate; the legacy snapshotDb() never ran in production because it lives on the silenced report cron). Protects against corruption/bad migration/deletion; volume loss still needs Railway volume backups or an off-platform copy. Shipped 2026-07-12. |
...
| `MODEL_AUDIT` | Model for the nightly audit (default `claude-sonnet-4-6`; must be a prefix the cost tracker recognizes). Set to `claude-sonnet-4-6` on Railway (2026-07-06). |
...
- Live Railway env (2026-07-12): `MODEL_CLASSIFIER=claude-haiku-4-5-20251001` (cost trial, ~1/3 the per-call price); reply, teacher, owner_qa, audit, and `MODEL_IMAGE_DESCRIBE` on `claude-sonnet-4-6`. Code-level fallback default stays `claude-sonnet-4-6` for every call site (2026-07-06 revert), so unsetting a var always rolls back safely. Every call passes `thinking: { type: 'disabled' }` (harmless on 4.6, and guards any future model that runs adaptive thinking by default). Classifier `max_tokens` is 1000 (raised from 400 on 2026-07-06 after truncated JSON caused paid parse_fail retries).
...
| File | Role |
|---|---|
| `src/owner_qa.js` + `src/prompts/owner_qa.md` | Owner Q&A mode. Brother WhatsApps Sunny questions about his data, gets answers from a live snapshot (today's stats, last 24h hot leads, pending queries, recent contacts, brother's own chat history, active facts count, AND `lead_routing` since 2026-06-08). **2026-07-05 capability layer (owner directive: answer ANYTHING the owners ask, in detail):** deterministic fast-paths in `handleOwnerNonQueryMessage` send a real datasheet file on a datasheet ask and a verbatim chat transcript on "send his chat" (target = explicit phone in the message, else the last customer number in the owner's own thread); any other question naming a customer gets a customer-in-focus block (contact details + 30-turn transcript) injected via `answerOwnerQuestion(id, q, { extraContext })`; `max_tokens` 1500; replies chunked at 3500 chars. Owner voice notes are transcribed into Owner Q&A. Owner non-text messages NEVER reach the customer pipeline (unconditional `continue`), and `scrubTeamContactLeadTags()` at boot nulls lead tags on team contacts (Charbel-tagged-HOT incident). `buildRoutingSummary(configuredRecipients())` produces the `lead_routing` block so Owner Q&A answers routing questions factually (routing is active, which Abuja/Lagos desks are set, how leads forward by city) instead of guessing that forwarding "needs to be set up". **2026-07-12 capability-awareness fix:** `buildMediaCoverageSummary(warehouse.listItems())` produces a `media_coverage` block (datasheet/photo auto-send is BUILT and LIVE; counts + exact items missing files), and `owner_qa.md` gained a "What you can already do" section plus hard rules against inventing feature requests or promising code fixes. Root cause: Sunny told the developer line it needed "a media library endpoint" for datasheets/photos when both fast-paths exist; the flagged requests were items with no file uploaded (all three panels, racks, PDUs). |
...
| `src/handler.js > detectLargeOrderNgn(text)` / `isBigProjectByValue(text)` | Big-project-by-value detector (2026-07-05). Returns the largest Naira figure in `text` (only figures tied to ₦/NGN/naira/"million"; never SKU/wattage/kWh) / whether it clears `BIG_PROJECT_NGN_THRESHOLD` (default ₦15M). In `processCustomerBatch` it scans `safeCombinedText` + the last 4 assistant turns (Sunny's own BOM) and sets `classification.routing_category='big_project'` so `decideRecipient` routes to the OWNERS (Patrick/Charbel round-robin) regardless of region. Exported + unit-tested (`test/big_project_value.test.js`). |
| `src/handler.js` Sales-Manager-referral invariant (post-reply) | (2026-07-05) After the reply is finalized, if it names "Sales Manager"/"specialist" and no owner/desk alert fired this turn, force one (`source='sales_manager_referral'`, routed by the same rules so a big project reaches an owner), clear a stale gather-first deferral, and ALWAYS append the direct line (`buildSpecialistLink`, falls back to `SPECIALIST_DIRECT_LINK`). Both the HOT and generic handoff backstops also had their `!gatherFirst` guard removed so a promised handoff always alerts. Guarantees no Sales-Manager mention goes silent. |
| `src/claude.js > describeInboundImage` + image plumbing in `src/handler.js` | Image-reading fix (2026-07-12; owner report "he's not reading images"; traced live: a customer's "I mean this one" + product photo got total silence). Every downloaded customer image gets ONE strictly-descriptive vision call (`MODEL_IMAGE_DESCRIBE`, defaults to MODEL_CLASSIFIER, pinned to Sonnet 4.6 on Railway; 200 tokens, legible text quoted verbatim, nothing guessed, budget-guarded, null on failure -> legacy marker). The description rides in the persisted body (`[image] cap` + `[Image content: ...]` via `buildImagePersistedBody`) and the classifier input (`buildImageCombinedPart`), so classification, owner alerts, pending queries, and future history turns all see what the image shows; the reply model additionally receives the raw image as vision (unchanged). Image-bearing turns BYPASS the pending-query silence cooldown (`shouldSuppressFollowupReply`, extracted + unit-tested), and the awaiting-expert context gains a `hasImage` instruction (answer what the image shows, match legible models against warehouse stock, never guess specs). `classifier.md`: an attached image is never by itself an escalation. Tests: `test/image_reading.test.js`. |
| `src/db_backup.js` | Nightly DB backup (2026-07-12). `runDbBackup()`: better-sqlite3 online backup (WAL-safe) -> gzip -> `DB_BACKUP_DIR`, prune to newest `DB_BACKUP_KEEP`. Cron `10 2 * * *` Africa/Lagos in server.js, ALWAYS registered. Logs `db_backup.ok` / `db_backup.fail`. Tests: `test/db_backup.test.js` (gunzip + reopen verified). |
| `src/idle_chatter.js` + `src/handler.js > maybeMuteIdleChatter` | Idle-chatter mute (2026-07-11 owner directive: stop replying to unproductive conversations; hardened 2026-07-15 after a live spam thread bypassed it). Pure core `classifyLowValue(text)` tags a message `bare_link` / `emoji_only` / `non_serviced_script` / `unintelligible` / `courtesy` or null; `assessIdleChatter` counts the trailing streak of consecutive low-value customer turns and mutes once the polite-reply allowance (`IDLE_CHATTER_FREE_REPLIES`, default 1) is used up. Bare junk links never get a reply at all, INCLUDING share-cards whose non-URL text is itself low-value (the Jul 14 Arabic TikTok share). `courtesy` (pure thanks/greetings) extends a streak but never resets it, and only mutes when the streak contains hard junk. Transliterated-Arabic chatter (romanized Whisper output, "ya habibi...") is caught by a strong/weak Latin-token lexicon. Script-ratio threshold 0.5, computed on URL-stripped text. The handler wrapper runs in `processCustomerBatch` BEFORE the classifier (muted turn = zero LLM calls), writes the `silent_skip` marker (orphan-sweep safe) + `idle_chatter_muted` event. Never mutes: image-attachment turns, any message with a product token or digit (all five serviced languages are Latin script and real leads always carry product words or figures). Tests: `test/idle_chatter.test.js` (incl. the full Jul 14 spam sequence as a regression), `test/idle_chatter_wiring.test.js`. |
| `src/handler.js > recoverOrphanedInbound(maxAgeMinutes)` | Scans inbound messages without a subsequent outbound reply (and not human_handled, not from owner, not reactions) and re-queues them through the normal pipeline. Called 3s after `app.listen` (10 min lookback) and on the `*/5` cron (360 min lookback, `minAgeMinutes: 3`). Bug it fixes: in-memory debounce queue is wiped on container restart. **2026-07-06 cost-runaway hardening:** (a) each message gets at most `ORPHAN_RECOVERY_MAX_ATTEMPTS` (default 2) re-queues, tracked in-memory per msg_id (`resetOrphanRecoveryAttempts()` for tests; logs `handler.recovery.attempts_capped`); (b) deliberate-silence turns write a non-sent outbound marker via `persistSilentSkipMarker(conversationId, note)` (`intent='silent_skip'`, no whatsapp_message_id) so they stop looking like unanswered customers; the warm-close casual-confirm skip writes it; (c) the "answered" check is `m2.timestamp >= m.timestamp` (same-millisecond outbound counts); (d) `opts.enqueue` injection for tests (`test/orphan_recovery.test.js`). Background: the sweep was re-queuing the same "Ok"/"🙏🏼" warm-close messages every 5 min for 6h, ~600 extra classifier calls and ~$11 on 2026-07-06. |
...
| `src/vault.js` + `vault/` | Knowledge vault (2026-07-17). Selective topic retrieval: `vault/tag-map.json` maps topic tags -> markdown files (`products/`, `policies/`, `playbook/`) with descriptions (fed to the classifier) and fallback keywords (deterministic backup when the classifier returns no tags). `buildClassifierTagBlock()` = third cached classifier system block requesting `topic_tags` in the JSON (code-built, survives owner prompt rewrites). `buildKnowledgeBlock(tags, msg)` = uncached `<business_knowledge>` reply block, only matching files, `%%` comment blocks + `[TODO` lines stripped (all-TODO file injects nothing), budget-capped (`VAULT_PROMPT_BUDGET_TOKENS` 1000 tokens, line-boundary truncation), path-traversal safe, fail-open (any error -> empty block, reply proceeds). Reply block sits AFTER the 4 cached blocks (which already use all 4 cache breakpoints; per-message content must never join the cache prefix). Prices/stock NEVER come from the vault (preamble + owner editing rules in `VAULT.md`). Tests: `test/vault.test.js`. |
...
- **LLM**: Anthropic Claude API. Live models (2026-07-12): classifier on `claude-haiku-4-5-20251001` via `MODEL_CLASSIFIER` (cost trial); reply, teacher, owner Q&A, audit, and image descriptions (`MODEL_IMAGE_DESCRIBE`) on `claude-sonnet-4-6`. CODE defaults for all call sites (`src/claude.js > classify`, `generateReply`, `describeInboundImage`; `src/knowledge.js > extractKnowledge`; `src/owner_qa.js > answerOwnerQuestion`; `src/audit.js`) remain `claude-sonnet-4-6`, so unsetting any env var is a safe rollback. All calls pass `thinking: { type: 'disabled' }`. Prompt caching enabled on system blocks via `cache_control: { type: 'ephemeral', ttl: '1h' }` (1-hour TTL since 2026-05-30; see Models, costs, and budget).
...
│   ├── vault.js                 # Knowledge-vault loader: topic tags -> selective file retrieval, budget-capped injection.
...
├── vault/                       # Knowledge vault: tag-map.json + products/ + policies/ + playbook/ markdown (see VAULT.md).
├── VAULT.md                     # Owner-facing guide to the knowledge vault.
├── presentation/
...
5. `handler.js > extractMessages` extracts text, image, audio, and emoji reactions (a `reaction` is tagged `kind: 'reaction'` and routed to `handleReaction`, which persists it silently and never replies). `handleInbound(payload)`:
   ...
   3. **Images**: download from Meta, save to `MEDIA_DIR`, base64-encode, persist with `media_path` and `media_mime`. Since 2026-07-12 each image also gets a strictly-descriptive vision call (`describeInboundImage`); the description is appended to the persisted body and the classifier input as `[Image content: ...]`, so the text-only classifier, owner alerts, and future history see what the image shows. The reply call additionally sees the actual image as a vision input. Image-bearing turns bypass the pending-query silence cooldown and get a `hasImage` awaiting-expert instruction when escalated.
   ...
   7b. **Idle-chatter mute** (2026-07-11): before the classifier, `maybeMuteIdleChatter` checks whether the batch is low-value (non-serviced-script chatter, emoji-only, bare junk link, dot transcript) and whether the conversation's polite-reply allowance (`IDLE_CHATTER_FREE_REPLIES`, default 1) is already spent. If so: no classifier, no reply, a `silent_skip` marker is written and the turn ends. Bare junk links mute from the first message. Substantive messages and image turns always fall through.
...
| `MODEL_CLASSIFIER`, `MODEL_REPLY`, `MODEL_TEACHER`, `MODEL_OWNER_QA`, `MODEL_IMAGE_DESCRIBE` | Per-call-site model overrides. Live: classifier on `claude-haiku-4-5-20251001`, everything else `claude-sonnet-4-6`. |
...
- `escalation_type`: hot_lead / silent_query / negotiation / repeat_complex / big_project / bulk_order / pricing_question / none. `bulk_order` (2026-06-07) is set in code (`handler.js > detectBulkQuantity`, threshold qty ≥ 2) when a non-HOT customer names a product + multi-unit quantity; it bypasses gather-first, quotes the per-unit price + offers the Sales Manager for the bulk price (`buildBulkOrderContext`), alerts the routed owner, and appends the routed Sales Manager direct line. (Routing for negotiation/repeat_complex still falls through silent_query flow; only the alert header label is differentiated. **big_project now routes to the OWNERS** when detected by BOM value: `handler.js > isBigProjectByValue` sets `routing_category='big_project'` for orders ≥ `BIG_PROJECT_NGN_THRESHOLD`, so `decideRecipient` picks Patrick/Charbel round-robin regardless of region, 2026-07-05.)
...
- **Sonnet on replies, Haiku on the classifier** (since 2026-07-12; the 2026-05-09 Opus-reply split ended with the 2026-07-06 all-Sonnet revert): reply ~$0.005-$0.01 per message on Sonnet 4.6; classifier ~$0.002 per message on Haiku 4.5 ($1/$5 per MTok). Baseline spend ~$21/day at current traffic before the Haiku switch, expected ~$3-5/day lower after. Use `MODEL_*` env overrides to step any call site up or down.
